...
Sylius console runs in ``dev`` environment by default. You can access the website in dev mode via the ``/app_dev.php`` file in the ``public/`` directory. (under your website root)
...
You can access the website in production mode via the ``/app.php`` file in your website root (``public/``) or just ``/`` path. (on Apache)
...
You can access the website in staging mode via the ``/app_staging.php`` file in your website root (``public/``) or just ``/`` path. (on Apache)
