...
* ``1.3``, ``1.4`` or ``1.5``, if you are fixing a bug for an existing feature or want to make a change that falls into the list of acceptable changes in patch versions
* ``master``, if you are adding a new feature.

.. note::

    All bug fixes merged into the ``1.3``, ``1.4`` and ``1.5`` maintenance branches are also merged into ``master`` on a regular basis.
