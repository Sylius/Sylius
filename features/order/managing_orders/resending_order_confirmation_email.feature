...
Feature: Resending an order confirmation email for a chosen order
    ...
    @ui @email @api
    Scenario: Resending a confirmation email for a given order in different locale than the default one
    ...
    @ui @email @api
    Scenario: Not being able to resend a confirmation email for a given order with wrong state
        When I view the summary of the order "#00000666"
