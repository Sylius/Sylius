...
Feature: Modifying a customer's billing address on an order with taxes
    ...
    As an Administrator
    I want to be able to modify a customer's billing address without changing the order's total
    ...
    Background:
        ...
        And the store has a "VAT" tax rate of 20% for "Suits" and "General" customer tax category within the "US" zone
