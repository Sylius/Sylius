...
Feature: Seeing shipping total of an order
    ...
    Background:
        ...
        And the store has "VAT" tax rate of 23% for "Hogwart stuff" within the "US-TAX" zone
        And the store has "Shipping VAT" tax rate of 23% for "Shipping Services" within the "US-TAX" zone
        And the store has a product "Gryffindor scarf" priced at "$100.00"
        And it belongs to "Hogwart stuff" tax category
        And the store has "Owl post" shipping method with "$10.00" fee within the "US-SHIPPING" zone
