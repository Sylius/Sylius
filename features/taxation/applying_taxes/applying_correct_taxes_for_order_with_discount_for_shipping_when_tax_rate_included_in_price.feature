...
Feature: Apply correct taxes for an order with a discount for a shipping when tax rates are included in price
    ...
    Background:
        ...
        And the store has included in price "Low VAT" tax rate of 10% for "Shipping" and "General" customer tax category within the "US" zone
    ...
    Scenario: Properly calculated taxes when item belongs to different tax category and has tax included in price
        Given the store has included in price "Standard VAT" tax rate of 23% for "Mugs" and "General" customer tax category within the "US" zone
