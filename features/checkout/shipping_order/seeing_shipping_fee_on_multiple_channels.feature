...
Feature: Seeing detailed shipping fee on multiple channels with different base currency
    ...
    Background:
        Given the store operates on a channel named "Web-US" in "USD" currency and with hostname "WebUSA"
        And the store operates on another channel named "Web-GB" in "GBP" currency and with hostname "WebGB"
