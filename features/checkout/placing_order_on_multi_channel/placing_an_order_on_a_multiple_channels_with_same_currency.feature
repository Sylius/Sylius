...
Feature: Placing an order on multiple channels with same currency
    ...
    Background:
        Given the store operates on a channel named "Los Angeles" in "USD" currency
        And the store operates on another channel named "New York" in "USD" currency
    ...
    Scenario: Placing an order in a channels base currency
        Given I changed my current channel to "Los Angeles"
    ...
    Scenario: Placing an order on a different channel with same currency
        Given I changed my current channel to "New York"
