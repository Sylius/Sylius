...
Feature: Placing an order on multiple channels with different currency
    ...
    Background:
        Given the store operates on a channel named "United States" in "USD" currency and with hostname "USA"
        And the store operates on another channel named "Colombia" in "COP" currency and with hostname "Colombia"
