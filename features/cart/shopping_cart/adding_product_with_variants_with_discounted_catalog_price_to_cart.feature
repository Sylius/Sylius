...
Feature: Adding a product with selected variant with discounted catalog price to the cart
    ...
    Background:
        ...
        And the product "T-Shirt" has a "PHP T-Shirt" variant originally priced at "$20"
        And the product "T-Shirt" has a "Kotlin T-Shirt" variant originally priced at "$400"
        And the store has a "Keyboard" configurable product
        And the product "Keyboard" has a "RGB Keyboard" variant originally priced at "$40"
        And the product "Keyboard" has a "Pink Keyboard" variant originally priced at "$40"
