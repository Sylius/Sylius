...
class InventoryUnitSpec extends ObjectBehavior
{
    ...
    public function it_implements_Sylius_core_inventory_unit_interface()
    {
        $this->shouldImplement('Sylius\Component\Core\Model\InventoryUnitInterface');
    }

    public function it_extends_Sylius_inventory_unit_model()
    {
    ...
    public function it_does_not_belong_to_any_shipment_by_default()
    {
        $this->getShipment()->shouldReturn(null);
    }

    public function it_allows_assigning_itself_to_a_shipment(ShipmentInterface $shipment)
    {
        $this->setShipment($shipment);
        $this->getShipment()->shouldReturn($shipment);
    }

    public function it_allows_detaching_itself_from_a_shipment(ShipmentInterface $shipment)
    {
        $this->setShipment($shipment);
        $this->getShipment()->shouldReturn($shipment);
    ...
    public function it_does_not_belong_to_an_order_item_by_default()
    {
        $this->getOrderItem()->shouldReturn(null);
    }

    public function it_allows_attaching_itself_to_an_order_item(OrderItemInterface $order_item)
    {
        $this->setOrderItem($order_item);
        $this->getOrderItem()->shouldReturn($order_item);
    }

    public function it_allows_detaching_itself_from_an_order_item(OrderItemInterface $order_item)
    {
        $this->setOrderItem($order_item);
        $this->getOrderItem()->shouldReturn($order_item);
