...
final class UnitFixedDiscountPromotionActionCommandTest extends TestCase
{
    ...
    private UnitFixedDiscountPromotionActionCommand $command;
    ...
    protected function setUp(): void
    {
        ...
        $this->command = new UnitFixedDiscountPromotionActionCommand(
    ...
    public function testShouldBeDiscountAction(): void
    {
        $this->assertInstanceOf(UnitDiscountPromotionActionCommand::class, $this->command);
    }

    public function testShouldApplyFixedDiscountOnEveryUnitInOrder(): void
    {
        ...
        $this->priceRangeFilter
            ->expects($this->once())
            ->method('filter')
            ->with([$this->orderItem], ['amount' => 500, 'channel' => $this->channel])
            ->willReturn([$this->orderItem]);
        $this->taxonFilter
            ->expects($this->once())
            ->method('filter')
            ->with([$this->orderItem], ['amount' => 500])
            ->willReturn([$this->orderItem]);
        $this->productFilter
            ->expects($this->once())
            ->method('filter')
        ...
        $this->adjustmentFactory
            ...
            ->method('createNew')
            ->willReturnOnConsecutiveCalls($this->firstPromotionAdjustment, $this->secondPromotionAdjustment);

        $this->firstUnit->expects($this->exactly(2))->method('getTotal')->willReturn(1000);
        ...
        $this->firstPromotionAdjustment->expects($this->once())->method('setAmount')->with(-500);
        ...
        $this->secondUnit->expects($this->exactly(2))->method('getTotal')->willReturn(1000);
        $this->secondPromotionAdjustment
            ->expects($this->once())
            ->method('setType')
            ->with(AdjustmentInterface::ORDER_UNIT_PROMOTION_ADJUSTMENT);
        $this->secondPromotionAdjustment->expects($this->once())->method('setLabel')->with('Test promotion');
        $this->secondPromotionAdjustment->expects($this->once())->method('setAmount')->with(-500);
        ...
        $this->assertTrue(
            $this->command->execute($this->order, ['WEB_US' => ['amount' => 500]], $this->promotion),
        );
    }

    public function testShouldApplyFixedDiscountOnlyToNonDiscountedUnitsIfPromotionDoesNotApplyToDiscountedOnes(): void
    {
        ...
        $this->priceRangeFilter
            ->expects($this->once())
            ->method('filter')
            ->with([$this->firstOrderItem, $this->secondOrderItem], ['amount' => 500, 'channel' => $this->channel])
            ->willReturn([$this->firstOrderItem, $this->secondOrderItem]);
        $this->taxonFilter
            ->expects($this->once())
            ->method('filter')
            ->with([$this->firstOrderItem, $this->secondOrderItem], ['amount' => 500])
            ->willReturn([$this->firstOrderItem, $this->secondOrderItem]);
        $this->productFilter
            ->expects($this->once())
            ->method('filter')
        ...
        $this->secondUnit->expects($this->once())->method('getTotal')->willReturn(1000);
        $this->secondUnit->expects($this->once())->method('getOrderItem')->willReturn($this->secondOrderItem);
        $this->firstProductVariant
            ->expects($this->once())
            ->method('getChannelPricingForChannel')
        ...
        $this->adjustmentFactory
            ->expects($this->once())
            ->method('createNew')
            ->willReturn($this->firstPromotionAdjustment);
        $this->firstPromotionAdjustment
            ->expects($this->once())
            ->method('setType')
            ->with(AdjustmentInterface::ORDER_UNIT_PROMOTION_ADJUSTMENT);
    ...
    public function testShouldNotApplyDiscountIfAllItemsHaveBeenFilteredOut(): void
    {
        ...
        $this->priceRangeFilter
            ->expects($this->once())
            ->method('filter')
            ->with([$this->firstOrderItem], ['amount' => 500, 'channel' => $this->channel])
            ->willReturn([$this->firstOrderItem]);
        $this->taxonFilter
            ->expects($this->once())
            ->method('filter')
            ->with([$this->firstOrderItem], ['amount' => 500])
            ->willReturn([$this->firstOrderItem]);
        $this->productFilter
            ->expects($this->once())
            ->method('filter')
    ...
    public function testShouldNotApplyDiscountWithAmountZero(): void
    {
        ...
        $this->assertFalse(
            $this->command->execute($this->order, ['WEB_US' => ['amount' => 0]], $this->promotion),
        );
    }

    public function testShouldNotApplyBiggerDiscountThanUnitTotal(): void
    {
        $this->order->expects($this->exactly(4))->method('getChannel')->willReturn($this->channel);
        $this->channel->expects($this->once())->method('getCode')->willReturn('WEB_US');
        $this->firstUnit->expects($this->once())->method('getOrderItem')->willReturn($this->firstOrderItem);
        ...
        $this->secondChannelPricing->expects($this->once())->method('getMinimumPrice')->willReturn(0);
        $this->order->expects($this->once())->method('getItems')->willReturn(new ArrayCollection([$this->orderItem]));
        $this->priceRangeFilter->expects($this->once())->method('filter')
            ->with([$this->orderItem], ['amount' => 1000, 'channel' => $this->channel])
            ->willReturn([$this->orderItem]);
        $this->taxonFilter->expects($this->once())->method('filter')
            ->with([$this->orderItem], ['amount' => 1000])
            ->willReturn([$this->orderItem]);
        $this->productFilter->expects($this->once())->method('filter')
            ->with([$this->orderItem], ['amount' => 1000])
            ->willReturn([$this->orderItem]);
        $this->orderItem->expects($this->once())->method('getUnits')
            ->willReturn(new ArrayCollection([$this->firstUnit, $this->secondUnit]));
        $this->promotion->expects($this->exactly(2))->method('getName')->willReturn('Test promotion');
        $this->promotion->expects($this->exactly(2))->method('getCode')->willReturn('TEST_PROMOTION');
        $this->promotion->expects($this->exactly(2))->method('getAppliesToDiscounted')->willReturn(true);
        $this->adjustmentFactory->expects($this->exactly(2))->method('createNew')
            ->willReturnOnConsecutiveCalls($this->firstPromotionAdjustment, $this->secondPromotionAdjustment);
        $this->firstUnit->expects($this->exactly(2))->method('getTotal')->willReturn(300);
        $this->firstPromotionAdjustment
            ->expects($this->once())
            ->method('setType')
            ->with(AdjustmentInterface::ORDER_UNIT_PROMOTION_ADJUSTMENT);
        $this->firstPromotionAdjustment->expects($this->once())->method('setLabel')->with('Test promotion');
        $this->firstPromotionAdjustment->expects($this->once())->method('setAmount')->with(-300);
        $this->firstPromotionAdjustment->expects($this->once())->method('setOriginCode')->with('TEST_PROMOTION');
        $this->firstUnit->expects($this->once())->method('addAdjustment')->with($this->firstPromotionAdjustment);
        $this->secondUnit->expects($this->exactly(2))->method('getTotal')->willReturn(200);
        $this->secondPromotionAdjustment
            ->expects($this->once())
            ->method('setType')
            ->with(AdjustmentInterface::ORDER_UNIT_PROMOTION_ADJUSTMENT);
        $this->secondPromotionAdjustment->expects($this->once())->method('setLabel')->with('Test promotion');
        $this->secondPromotionAdjustment->expects($this->once())->method('setAmount')->with(-200);
        $this->secondPromotionAdjustment->expects($this->once())->method('setOriginCode')->with('TEST_PROMOTION');
        $this->secondUnit->expects($this->once())->method('addAdjustment')->with($this->secondPromotionAdjustment);

        $this->assertTrue(
            $this->command->execute($this->order, ['WEB_US' => ['amount' => 1000]], $this->promotion),
        );
    }

    public function testShouldNotApplyBiggerDiscountThanVariantMinimumPrice(): void
    {
        $this->order->expects($this->exactly(5))->method('getChannel')->willReturn($this->channel);
        $this->channel->expects($this->once())->method('getCode')->willReturn('WEB_US');
        $this->firstUnit->expects($this->once())->method('getOrderItem')->willReturn($this->firstOrderItem);
        $this->secondUnit->expects($this->exactly(2))->method('getOrderItem')->willReturn($this->firstOrderItem);
        $this->firstOrderItem->expects($this->exactly(3))->method('getVariant')->willReturn($this->firstProductVariant);
        $this->firstOrderItem->expects($this->exactly(3))->method('getOrder')->willReturn($this->order);
        $this->firstProductVariant->expects($this->exactly(3))->method('getChannelPricingForChannel')
            ->with($this->channel)->willReturn($this->firstChannelPricing);
        $this->firstChannelPricing->expects($this->exactly(3))->method('getMinimumPrice')->willReturn(200);
        ...
        $this->priceRangeFilter->expects($this->once())->method('filter')
            ->with([$this->firstOrderItem, $this->secondOrderItem], ['amount' => 1000, 'channel' => $this->channel])
            ->willReturn([$this->firstOrderItem, $this->secondOrderItem]);
        $this->taxonFilter->expects($this->once())->method('filter')
            ->with([$this->firstOrderItem, $this->secondOrderItem], ['amount' => 1000])
            ->willReturn([$this->firstOrderItem, $this->secondOrderItem]);
        $this->productFilter->expects($this->once())->method('filter')
            ->with([$this->firstOrderItem, $this->secondOrderItem], ['amount' => 1000])
            ->willReturn([$this->firstOrderItem, $this->secondOrderItem]);
        $this->firstOrderItem->expects($this->once())->method('getUnits')
            ->willReturn(new ArrayCollection([$this->firstUnit, $this->secondUnit]));
        $this->secondOrderItem->expects($this->once())->method('getUnits')
            ->willReturn(new ArrayCollection([$this->secondUnit]));
        $this->promotion->expects($this->exactly(3))->method('getName')->willReturn('Test promotion');
        $this->promotion->expects($this->exactly(3))->method('getCode')->willReturn('TEST_PROMOTION');
        $this->adjustmentFactory->expects($this->exactly(3))->method('createNew')
        ...
        $this->firstPromotionAdjustment
            ...
            ->method('setType')
            ->with(AdjustmentInterface::ORDER_UNIT_PROMOTION_ADJUSTMENT);
        $this->firstPromotionAdjustment->expects($this->exactly(2))->method('setLabel')->with('Test promotion');
        $this->firstPromotionAdjustment->expects($this->exactly(2))->method('setAmount')->with(-100);
        $this->firstPromotionAdjustment->expects($this->exactly(2))->method('setOriginCode')->with('TEST_PROMOTION');
        $this->firstUnit->expects($this->once())->method('addAdjustment')->with($this->firstPromotionAdjustment);
        $this->secondUnit->expects($this->exactly(4))->method('getTotal')->willReturn(300);
        $this->secondPromotionAdjustment
            ->expects($this->once())
            ->method('setType')
            ->with(AdjustmentInterface::ORDER_UNIT_PROMOTION_ADJUSTMENT);
        ...
        $this->secondPromotionAdjustment->expects($this->once())->method('setOriginCode')->with('TEST_PROMOTION');
        $this->secondUnit->expects($this->exactly(2))->method('addAdjustment')->with($this->secondPromotionAdjustment);

        $this->assertTrue($this->command->execute($this->order, ['WEB_US' => ['amount' => 1000]], $this->promotion));
    }

    public function testShouldNotApplyDiscountIfNoAmountIsDefinedForOrderChannel(): void
    {
        $this->order->expects($this->once())->method('getChannel')->willReturn($this->channel);
        $this->channel->expects($this->once())->method('getCode')->willReturn('WEB_US');
        $this->adjustmentFactory->expects($this->never())->method('createNew');
        ...
        $this->assertFalse(
            $this->command->execute($this->order, ['WEB_PL' => ['amount' => 0]], $this->promotion),
        );
    }

    public function testShouldThrowExceptionIfPassedSubjectToExecuteIsNotOrder(): void
    {
    ...
    public function testShouldRevertProperPromotionAdjustmentFromAllUnits(): void
    {
        $this->order->expects($this->once())->method('getItems')->willReturn(new ArrayCollection([
            $this->firstOrderItem,
        ]));
        ...
        $this->command->revert($this->order, ['WEB_US' => ['amount' => 1000]], $this->promotion);
    }
