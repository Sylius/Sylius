...
namespace Sylius\ShippingBundle\DependencyInjection\Compiler;
...
class RegisterRuleCheckersPass implements CompilerPassInterface
{
    ...
    {
        if (!$container->hasDefinition('sylius.registry.shipping_rule_checker')) {
            return;
        }

        $registry = $container->getDefinition('sylius.registry.shipping_rule_checker');
        $checkers = [];
