...
final class RegisterReviewFactoryPass implements CompilerPassInterface
{
    ...
    {
        $reviewSubjects = $container->getParameter('sylius.review.subjects');
        if (!is_iterable($reviewSubjects)) {
            return;
        }
        foreach ($reviewSubjects as $subject => $configuration) {
            ...
            $container->setDefinition(sprintf('sylius.factory.%s_review', $subject), $reviewFactoryDefinition);
        }
    }
}
