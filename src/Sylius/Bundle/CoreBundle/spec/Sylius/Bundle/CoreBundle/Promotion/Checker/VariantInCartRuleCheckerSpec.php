 ...
 */
class VariantInCartRuleCheckerSpec extends ObjectBehavior
{
    function it_should_be_initializable()
    {
        $this->shouldHaveType('Sylius\Bundle\CoreBundle\Promotion\Checker\VariantInCartRuleChecker');
    }
    ...
    {
        $variant->getId()->shouldBeCalled()->willReturn(2);
        $item->getVariant()->shouldBeCalled()->willReturn($variant);
        $subject->getItems()->shouldBeCalled()->willReturn(array($item));

        $this->isEligible($subject, array('variant' => 1))->shouldReturn(false);
    }
}
