...
final class TimeoutRunnerTest extends TestCase
{
    public function test_mysql_applies_optimizer_hint(): void
    {
        ...
        $connection->method('getDatabasePlatform')->willReturn($this->createMock(MySqlPlatform::class));
    ...
    {
        $connection = $this->createMock(Connection::class);
        $connection->method('getDatabasePlatform')->willReturn($this->createMock(MariaDb1027Platform::class));
        $connection->expects(self::once())
            ->method('fetchAllAssociative')
            ->with(
