        ...
        $rootNode
            ...
            ->children()
                ...
                ->arrayNode('oauth')
                    ->addDefaultsIfNotSet()
                    ->children()
                        ->arrayNode('account_linking')
                            ->addDefaultsIfNotSet()
                            ->children()
                                ->booleanNode('require_verified_email')
                                    ->info('Require the OAuth provider to confirm that the e-mail address is verified before linking it with data already present in the shop.')
                                    ->defaultTrue()
                                ->end()
                                ->arrayNode('trusted_resource_owners')
                                    ->info('Resource owners, as configured in "hwi_oauth.resource_owners", whose e-mail addresses are trusted even though they return no verification claim.')
                                    ->prototype('scalar')->end()
                                    ->defaultValue([])
                                ->end()
                            ->end()
