        ...
        $rootNode
            ...
            ->children()
                ...
                ->arrayNode('telemetry')
                    ...
                    ->end()
                ->end()
                ->arrayNode('theme_assets')
                    ->addDefaultsIfNotSet()
                    ->validate()
                        ->ifTrue(static fn (array $v): bool => count($v['channels']) > 1 && !str_contains($v['build_name'], '{channel}'))
                        ->thenInvalid('The "sylius_core.theme_assets.build_name" pattern must contain "{channel}" placeholder when multiple channels are configured, otherwise builds for the same theme would collide.')
                    ->end()
                    ->validate()
                        ->ifTrue(static fn (array $v): bool => count($v['channels']) > 1 && !str_contains($v['output_path'], '{channel}'))
                        ->thenInvalid('The "sylius_core.theme_assets.output_path" pattern must contain "{channel}" placeholder when multiple channels are configured, otherwise builds for the same theme would overwrite each other.')
                    ->end()
                    ->children()
                        ->scalarNode('themes_dir')
                        ...
                        ->scalarNode('entrypoint')
                            ->defaultValue('assets/{channel}/entrypoint.js')
                            ->cannotBeEmpty()
                        ->end()
                        ->scalarNode('build_name')
                            ->defaultValue('theme.{code}.{channel}')
                            ->cannotBeEmpty()
                            ->validate()
                                ->ifTrue(static fn (string $v): bool => !str_contains($v, '{code}'))
                                ->thenInvalid('The "sylius_core.theme_assets.build_name" pattern must contain "{code}" placeholder, got %s.')
                            ->end()
                        ->end()
                        ->scalarNode('output_path')
                            ->defaultValue('%kernel.project_dir%/public/build/themes/{code}/{channel}')
                            ->cannotBeEmpty()
                            ->validate()
                                ->ifTrue(static fn (string $v): bool => !str_contains($v, '{code}'))
                                ->thenInvalid('The "sylius_core.theme_assets.output_path" pattern must contain "{code}" placeholder, got %s.')
                            ->end()
                        ->end()
                    ->end()
                ->end()
