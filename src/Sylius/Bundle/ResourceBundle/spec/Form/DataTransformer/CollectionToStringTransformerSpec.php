...
final class CollectionToStringTransformerSpec extends ObjectBehavior
{
    ...
    public function it_transforms_collection_to_string(): void
    {
        $this->transform(new ArrayCollection(['abc', 'def', 'ghi', 'jkl']))->shouldReturn('abc,def,ghi,jkl');
    }

    public function it_transforms_string_to_collection(): void
    {
        $this->reverseTransform('abc,def,ghi,jkl')->shouldBeLike(new ArrayCollection(['abc', 'def', 'ghi', 'jkl']));
    }

    public function it_throws_transformation_failed_exception_if_transform_argument_is_not_a_collection(): void
    {
        $this->shouldThrow(TransformationFailedException::class)->during('transform', [new \stdClass()]);
    }

    public function it_throws_transformation_failed_exception_if_transform_argument_is_not_a_string(): void
    {
        $this->shouldThrow(TransformationFailedException::class)->during('reverseTransform', [new \stdClass()]);
    }

    public function it_returns_empty_string_if_empty_collection_given(): void
    {
        $this->transform(new ArrayCollection())->shouldReturn('');
    }

    public function it_returns_empty_collection_if_empty_string_given(): void
    {
