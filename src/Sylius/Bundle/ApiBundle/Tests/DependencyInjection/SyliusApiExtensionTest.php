...
final class SyliusApiExtensionTest extends TestCase
{
    ...
    public function testPrependsApiPlatformMappingWithBothEndpointsEnabled(): void
    {
        ...
        $this->assertCount(3, $prependedConfig[0]['mapping']['paths']);
        $this->assertStringEndsWith('/Resources/config/api_platform/properties', $prependedConfig[0]['mapping']['paths'][0]);
        $this->assertStringEndsWith('/Resources/config/api_platform/resources/admin', $prependedConfig[0]['mapping']['paths'][1]);
        $this->assertStringEndsWith('/Resources/config/api_platform/resources/shop', $prependedConfig[0]['mapping']['paths'][2]);
    }
    ...
    public function testAlwaysMapsPropertiesPathWhenAnyEndpointIsEnabled(): void
    {
        foreach ([
            ['admin_enabled' => true, 'shop_enabled' => true],
            ['admin_enabled' => true, 'shop_enabled' => false],
            ['admin_enabled' => false, 'shop_enabled' => true],
        ] as $endpoints) {
            $container = new ContainerBuilder();
            $container->setParameter('kernel.bundles_metadata', [
                'SyliusApiBundle' => ['path' => __DIR__ . '/../../'],
            ]);
            $container->prependExtensionConfig('sylius_api', ['endpoints' => $endpoints]);

            $this->extension->prepend($container);

            $paths = $container->getExtensionConfig('api_platform')[0]['mapping']['paths'];

            $this->assertStringEndsWith('/Resources/config/api_platform/properties', $paths[0]);
        }
    }
}
