...
return static function (ContainerConfigurator $container) {
    ...
    $services->set('sylius_api.extractor.property_info.empty_property_list', EmptyPropertyListExtractor::class)
        ->tag('property_info.list_extractor', ['priority' => -2000]);

    $services->set('sylius_api.changer.payment_method', PaymentMethodChanger::class)
        ->args([
    ...
    $services->set('sylius_api.listener.api_cart_blamer', ApiCartBlamerListener::class)
        ->args([
    ...
    $services->set('sylius_api.listener.authentication_success', AuthenticationSuccessListener::class)
    ...
    $services->set('sylius_api.converter.iri_to_identifier', IriToIdentifierConverter::class)
        ->args([
    ...
    $services->set('sylius_api.mapper.address', AddressMapper::class)
        ->public();

    $services->alias(AddressMapperInterface::class, 'sylius_api.mapper.address');

    $services->set('sylius_api.checker.applied_coupon_eligibility', AppliedCouponEligibilityChecker::class)
        ->args([
    ...
    $services->set('sylius_api.modifier.order_address', OrderAddressModifier::class)
        ->public()
        ->args([
    ...
    $services->set('sylius_api.assigner.order_promotion_code', OrderPromotionCodeAssigner::class)
        ->public()
        ->args([
    ...
    $services->set('sylius_api.listener.admin_authentication_success', AdminAuthenticationSuccessListener::class)
        ->args([service('api_platform.symfony.iri_converter')])
        ->tag('kernel.event_listener', ['event' => 'lexik_jwt_authentication.on_authentication_success', 'method' => 'onAuthenticationSuccessResponse']);
};
