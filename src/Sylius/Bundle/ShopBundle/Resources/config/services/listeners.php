...
return static function (ContainerConfigurator $container) {
    ...
    $services->set('sylius_shop.listener.shop_cart_blamer', ShopCartBlamerListener::class)
        ->args([
    ...
    $services->set('sylius_shop.listener.customer_email_updater', CustomerEmailUpdaterListener::class)
        ->args([
    ...
    $services->set('sylius_shop.event_subscriber.shop_customer_account_sub_section_cache_control', ShopCustomerAccountSubSectionCacheControlSubscriber::class)
        ->args([service('sylius.section_resolver.uri_based')])
        ->tag('kernel.event_subscriber', ['event' => 'kernel.response']);

    $services->set('sylius_shop.listener.order_customer_ip', OrderCustomerIpListener::class)
        ->args([
    ...
    $services->set('sylius_shop.listener.order_complete', OrderCompleteListener::class)
    ...
    $services->set('sylius_shop.listener.user_registration', UserRegistrationListener::class)
        ->args([
    ...
    $services->set('sylius_shop.listener.order_integrity_checker', OrderIntegrityChecker::class)
        ->args([
            service('router'),
    ...
    $services->set('sylius_shop.listener.order_locale_assigner', OrderLocaleAssigner::class)
        ->args([service('sylius.context.locale')])
        ->tag('kernel.event_listener', ['event' => 'sylius.order.pre_complete', 'method' => 'assignLocale']);

    $services->set('sylius_shop.event_subscriber.session_cart', SessionCartSubscriber::class)
        ->args([
            service('sylius.context.cart'),
    ...
    $services->set('sylius_shop.listener.user_cart_recalculation', UserCartRecalculationListener::class)
        ->args([
            service('sylius.context.cart'),
    ...
    $services->set('sylius_shop.listener.user_impersonated', UserImpersonatedListener::class)
        ->args([
            service('sylius_shop.storage.cart_session'),
            service('sylius.context.channel'),
    ...
    $services->set('sylius_shop.listener.cart_item_add', CartItemAddListener::class)
        ->args([service('sylius.modifier.order')])
        ->tag('kernel.event_listener', ['event' => 'sylius.cart_item_add', 'method' => 'addToOrder']);

    $services->set('sylius_shop.listener.cart_item_remove', CartItemRemoveListener::class)
        ->args([service('sylius.modifier.order')])
