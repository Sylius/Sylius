...
class SettingsHelperSpec extends ObjectBehavior
{
    ...
    function it_should_return_settings_by_namespace($settingsManager, ParameterCollection $parameters)
    {
        $settingsManager->load('sylius_taxation')->shouldBeCalled()->willReturn($parameters);

        $this->getSettings('sylius_taxation')->shouldReturn($parameters);
    }
    ...
    function it_should_return_settings_parameter_by_namespace_and_name(
        ...
        $settingsManager->load('sylius_shipping')->shouldBeCalled()->willReturn($parameters);
        $parameters->get('price')->shouldBeCalled()->willReturn(19.99);
