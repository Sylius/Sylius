...
return static function (ContainerConfigurator $container) {
    ...
    $services->set('sylius_admin.listener.shipment_ship', ShipmentShipListener::class)
    ...
    $services->set('sylius_admin.listener.locale', LocaleListener::class)
        ->args([service('sylius.checker.locale_usage')])
        ->tag('kernel.event_listener', ['event' => 'sylius.locale.pre_delete', 'method' => 'preDelete']);

    $services->set('sylius_admin.listener.resource_delete_exception', ResourceDeleteExceptionListener::class)
        ->args([
            service('router'),
    ...
    $services->set('sylius_admin.listener.resource_delete', ResourceDeleteListener::class)
        ->tag('kernel.event_listener', ['event' => 'kernel.exception', 'method' => 'onResourceDelete']);

    $services->set('sylius_admin.event_subscriber.admin_section_cache_control', AdminSectionCacheControlSubscriber::class)
        ->args([service('sylius.section_resolver.uri_based')])
        ->tag('kernel.event_subscriber', ['event' => 'kernel.response']);

    $services->set('sylius_admin.event_subscriber.admin_filter', AdminFilterSubscriber::class)
