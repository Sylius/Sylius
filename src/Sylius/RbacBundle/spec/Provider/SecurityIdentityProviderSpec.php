...
namespace spec\Sylius\RbacBundle\Provider;
...
class SecurityIdentityProviderSpec extends ObjectBehavior
{
    ...
    {
        $this->shouldHaveType('Sylius\RbacBundle\Provider\SecurityIdentityProvider');
    }
    ...
    function it_throws_exception_if_user_does_not_implement_identity_interface($tokenStorage, TokenInterface $token, UserInterface $user)
    {
        ...
        $this
            ->shouldThrow(new \InvalidArgumentException('User class must implement "Sylius\Rbac\Model\IdentityInterface".'))
            ->duringGetIdentity()
        ;
    }
}
