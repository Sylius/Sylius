...
run_command "app/console doctrine:migrations:migrate --env=test_cached --no-debug --no-interaction -q -vvv" || exit $?
run_command "app/console doctrine:schema:validate --env=test_cached --no-debug --no-interaction -vvv" || exit $?
